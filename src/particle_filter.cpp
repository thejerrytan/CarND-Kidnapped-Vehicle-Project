...
void
ParticleFilter::dataAssociation(std::vector<Map::single_landmark_s> landmarks, std::vector<LandmarkObs> &observations) {
  ...
  for (auto &observation : observations) {
    ...
    for (int j = 0; j < landmarks.size(); j++) {
      ...
      if (distTo < minDistTo) {
        minDistTo = distTo;
        observation.id = landmarks[j].id_i;
      }
    }
...
void ParticleFilter::updateWeights(double sensor_range, double std_landmark[],
  ...
  for (auto &particle: particles) {
    ...
    for (auto &obs: transformedObs) {
      ...
      const double exponent = (((x_obs - x_mu) * (x_obs - x_mu)) / (2 * sigma_x * sigma_x)) +
                              (((y_obs - y_mu) * (y_obs - y_mu)) / (2 * sigma_y * sigma_y));
...
void ParticleFilter::resample() {
  ...
  for (int i = 0; i < num_particles; i++) {
    ...
    SetAssociations(new_p, particles[random_int].associations, particles[random_int].sense_x,
                    particles[random_int].sense_y);
